...
import sticker3 from "@/assets/stickers/Asset 4.png";
import sticker4 from "@/assets/stickers/Asset 5.png";
...
function Index() {
  ...
  const dbHighlights: any[] = data?.highlights?.length ? data.highlights : t.highlights.items.map((it: any, i: number) => ({
    title: { en: it.title, kn: it.title },
    desc: { en: it.desc, kn: it.desc },
    orderIndex: i,
  }));
  const highlightsTitle = data?.siteContentMap?.highlights?.title || t.highlights.title;
  const highlightsSubtitle = data?.siteContentMap?.highlights?.subtitle || t.highlights.subtitle;

  const dbHomeFeatures = data?.homeFeatures || [];
  ...
  return (
    <Layout>
      {loading ? (
        ...
        <>
          {/* HERO */}
          <section className="relative lg:min-h-screen flex items-center overflow-hidden pb-6 md:pb-16">
            ...
            <div className="container mx-auto px-6 relative z-10 flex flex-col lg:grid lg:grid-cols-2 gap-2 lg:gap-12 items-center pt-10 lg:pt-0 lg:min-h-0">
          ...
          <section className="container mx-auto px-6 pt-10 pb-4 md:pb-20 relative overflow-hidden">
            ...
            >
              ...
              <p className="text-muted-foreground text-sm md:text-base">{highlightsSubtitle}</p>
            </motion.div>
            ...
            <div className="relative group/slider">
              ...
              >
                {highlightOrder.map((idx, i) => {
                  const dbItem = dbHighlights[idx];
                  const item = dbItem
                    ? { title: dbItem.title?.[lang] || dbItem.title?.en || t.highlights.items[idx].title,
                        desc:  dbItem.desc?.[lang]  || dbItem.desc?.en  || t.highlights.items[idx].desc }
                    : t.highlights.items[idx];
                  const sticker = stickerIcons[idx];
                  return (
                    <Link
                      key={i}
                      to={idx === 0 ? "/classes" : "/services"}
                      hash={idx === 0 ? undefined : idx === 2 ? "performance" : "workshops"}
          ...
          <section className="container mx-auto px-6 pt-4 pb-12 md:py-12 flex flex-col gap-16 md:gap-32">
            ...
            <div id="classes-details" className="scroll-mt-32">
              <h2 className="text-[26px] sm:text-3xl md:text-5xl font-display mb-12 text-center text-primary flex items-center justify-center gap-4">
                {dbHighlights[0]?.title?.[lang] || dbHighlights[0]?.title?.en || t.highlights.items[0].title}
              </h2>
                    ...
                    >
                      <div className="relative h-24 sm:h-32 md:h-48 overflow-hidden">
                        ...
                        <div className="absolute top-2 left-2 md:top-4 md:left-4">
                          <span className="px-2 py-0.5 md:px-3 md:py-1 bg-background/60 backdrop-blur-md border border-gold/30 text-[7px] md:text-[9px] font-bold text-foreground uppercase tracking-wider rounded-full">
                            {post.category[lang]}
                          </span>
                        </div>
                      </div>
